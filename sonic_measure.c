...
int add_measurement(measurement_collection_t *collection, const char *component, 
    ...
    if (collection->count >= collection->capacity) {
        ...
        if (!collection->measurements) return -1;
    }
...
/* Measure firmware/BIOS information */
int measure_firmware(measurement_collection_t *collection) {
    char hash[SHA256_HEX_SIZE];
    char *content;
...
/* Measure kernel information */
int measure_kernel(measurement_collection_t *collection) {
    char hash[SHA256_HEX_SIZE];
    char *content;
    struct utsname uts;
    ...
    if (uname(&uts) == 0) {
        ...
        snprintf(kernel_info, sizeof(kernel_info), "%s %s %s", 
...
/* Measure SONiC configuration files */
int measure_sonic_config(measurement_collection_t *collection) {
    char hash[SHA256_HEX_SIZE];
    ...
    for (int i = 0; sonic_files[i]; i++) {
        if (access(sonic_files[i], R_OK) == 0) {
            if (hash_file(sonic_files[i], hash) == 0) {
                char *basename = strrchr(sonic_files[i], '/');
                if (basename) basename++; else basename = (char*)sonic_files[i];
                
                add_measurement(collection, "sonic", "config", hash, basename);
            }
        }
    }
    
    /* Measure additional SONiC directories */
    DIR *dir;
    struct dirent *entry;
    char filepath[MAX_PATH];
    
    dir = opendir("/etc/sonic");
    if (dir) {
...
/* Measure routing and forwarding state */
int measure_routing(measurement_collection_t *collection) {
    char hash[SHA256_HEX_SIZE];
    char *content;
    
    log_message(LOG_INFO, "Measuring routing and forwarding...");
    
    /* Kernel routing table */
    content = execute_command("ip route show | sort");
    if (content) {
        if (hash_string(content, hash) == 0) {
            add_measurement(collection, "routing", "kernel_routes", hash, "ip_routes");
        }
        free(content);
    }
    
    /* IPv6 routes */
    content = execute_command("ip -6 route show | sort");
    if (content) {
        if (hash_string(content, hash) == 0) {
            add_measurement(collection, "routing", "ipv6_routes", hash, "ipv6_routes");
        }
        free(content);
    }
    
    /* ARP table */
    content = execute_command("ip neigh show | sort");
    if (content) {
        if (hash_string(content, hash) == 0) {
            add_measurement(collection, "routing", "arp_table", hash, "neighbor_table");
        }
        free(content);
    }
    
    /* FRR configuration and state */
    content = execute_command("vtysh -c 'show running-config' 2>/dev/null");
    ...
    /* BGP state */
    content = execute_command("vtysh -c 'show ip bgp summary' 2>/dev/null");
    if (content && strlen(content) > 0) {
        if (hash_string(content, hash) == 0) {
            add_measurement(collection, "routing", "bgp_state", hash, "bgp_summary");
        }
        free(content);
    }
...
int measure_services(measurement_collection_t *collection) {
    char hash[SHA256_HEX_SIZE];
    char *content;
    char command[512];
    ...
    /* Individual container measurements for critical SONiC services */
    const char *sonic_containers[] = {
        "swss", "syncd", "bgp", "teamd", "lldp", "snmp", "dhcp_relay", NULL
    };
    ...
    for (int i = 0; sonic_containers[i]; i++) {
        ...
        if (content && strlen(content) > 10) {  /* Valid response */
            if (hash_string(content, hash) == 0) {
    ...
    content = execute_command("docker exec redis redis-cli --raw keys '*' | sort | xargs -I {} docker exec redis redis-cli --raw get {} 2>/dev/null");
    if (content) {
        if (hash_string(content, hash) == 0) {
            add_measurement(collection, "service", "redis_state", hash, "redis_database");
        }
        free(content);
    }
...
int measure_hardware(measurement_collection_t *collection) {
    char hash[SHA256_HEX_SIZE];
    char *content;
    ...
    /* Memory info */
    content = read_file_content("/proc/meminfo");
    if (content) {
        if (hash_string(content, hash) == 0) {
            add_measurement(collection, "hardware", "memory", hash, "memory_info");
        }
        free(content);
    }
    
    /* PCI devices */
    content = execute_command("lspci -nn | sort");
    if (content) {
        if (hash_string(content, hash) == 0) {
            add_measurement(collection, "hardware", "pci", hash, "pci_devices");
        }
        free(content);
    }
...
int parse_components(const char *component_str, int *components) {
    ...
    for (int i = 0; i < 6; i++) components[i] = 0;
    ...
    while (token != NULL && count < 6) {
        if (strcmp(token, "firmware") == 0) components[0] = 1;
        else if (strcmp(token, "kernel") == 0) components[1] = 1;
        else if (strcmp(token, "sonic") == 0) components[2] = 1;
        else if (strcmp(token, "routing") == 0) components[3] = 1;
        else if (strcmp(token, "services") == 0) components[4] = 1;
        else if (strcmp(token, "hardware") == 0) components[5] = 1;
        else {
...
int main(int argc, char *argv[]) {
    ...
    char *json_file = NULL;
    char *log_path = NULL;
    int components[6] = {1, 1, 1, 1, 1, 1};  /* All enabled by default */
    int opt;
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
            ...
            if (i + 1 < argc) {
                parse_components(argv[++i], components);
            }
        }
    }
    
    /* Initialize logging */
    if (log_path && init_logging(log_path) != 0) {
        return 1;
    }
    ...
    if (components[0] && measure_firmware(collection) != 0) {
        log_message(LOG_WARN, "Firmware measurement failed");
    }
    
    if (components[1] && measure_kernel(collection) != 0) {
        log_message(LOG_WARN, "Kernel measurement failed");
    }
    
    if (components[2] && measure_sonic_config(collection) != 0) {
        log_message(LOG_WARN, "SONiC configuration measurement failed");
    }
    
    if (components[3] && measure_routing(collection) != 0) {
        log_message(LOG_WARN, "Routing measurement failed");
    }
    
    if (components[4] && measure_services(collection) != 0) {
        log_message(LOG_WARN, "Services measurement failed");
    }
    
    if (components[5] && measure_hardware(collection) != 0) {
